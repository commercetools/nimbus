...
> `*.messages.ts` dictionaries generated by the build pipeline. After adding or
> changing a `.i18n.ts` file, run **`pnpm extract-intl`** to (re)generate the
> `*.messages.ts` dictionaries — the import won't resolve until you do. See
> `docs/file-type-guidelines/i18n.md` for the full extract → Transifex → compile
> flow.
