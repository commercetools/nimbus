...
- **The scrollbar now auto-hides when idle by default.** It appears when the
  ...
  short idle delay; while the pointer stays inside, only scrolling brings it
  back — so a resting reader is not distracted. Wheel, touch, and keyboard
  scrolling keep working while it is hidden.
- **`variant` now selects the visual style:** `solid` (default — grey track,
  ...
  `hidden` (no track, only the thumb shows), and `glass` (translucent, frosted
