...
Nimbus is a comprehensive design system providing React UI components, design
tokens, icons, and documentation for building accessible, consistent user
interfaces. The system prioritizes:
...
- **Internationalization**: Built-in i18n support with @internationalized/string
  and Transifex integration
...
- **@commercetools/nimbus-i18n**: Internationalization
  ...
  - @internationalized/string integration
...
- **Package isolation**: Each package has independent TypeScript config and
  build output
- **Storybook testing**: Tests run against built bundle (`dist/`), not source
  files
...
- **CRITICAL**: Never import from barrel exports (`index.ts`) across components
...
- **React Aria imports**: Always prefix with `Ra`
...
- **Direct file imports**: Across components (`@/components/icon/icon.tsx`, NOT
  `@/components/icon`)
- **Barrel exports**: For utilities (`@/utils`, `@/hooks`)

**CRITICAL**: Never import from barrel exports (`index.ts`) when importing
across component directories to avoid circular chunk dependencies.
...
2. **Tier 2 - Slot-Based**: Multiple slots, React Aria integration (TextInput,
   NumberInput)
3. **Tier 3 - Compound**: Multiple coordinated parts with `.Root` (Menu, Dialog,
   Tabs)
4. **Tier 4 - Complex**: All of above plus generics, context, helpers
...
1. **Root components**: Split recipe variants → Extract style props → Forward
   both
...
- **Semantic Colors**: `primary`, `neutral`, `info`, `positive`, `warning`,
  `critical`
...
3. **Build Before Testing**: Storybook tests run against built bundle, not
   source
4. **Cross-Chunk Imports**: Must import from implementation files across
   components
5. **Compound Component Structure**: `.Root` must be first property in namespace
   object
...
- **Recipe Variants**: Never explicitly declare variants that inherit from slot
  props
...
- **CSS Variables**: Must namespace with component name (e.g.,
  `--accordion-font-size`)
