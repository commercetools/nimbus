# Specification: TextInputField Component

## Overview

The TextInputField component is a pre-composed field pattern that combines TextInput with FormField to provide a complete, accessible text input form field with an intuitive flat API. It encapsulates label, hint text, error messaging, and validation feedback in a single component, reducing boilerplate code for common text input scenarios.

**Component:** `TextInputField`
**Package:** `@commercetools/nimbus`
**Type:** Pattern component (pre-composed field wrapper)
**Pattern:** Combines FormField + TextInput composition
**Location:** `packages/nimbus/src/patterns/fields/text-input-field/`

## Purpose

Provide a simplified, high-level API for creating complete text input form fields without manually composing FormField and TextInput components. Maintains backwards compatibility with UI-Kit's TextField by supporting the same error format and localized error messages while offering an ergonomic interface for the most common form field use cases.

## Requirements

### Requirement: Component Composition
The component SHALL compose FormField.Root, FormField.Label, FormField.Input, FormField.Description, FormField.Error, FormField.InfoBox, and TextInput.

#### Scenario: Component structure
- **WHEN** TextInputField renders
- **THEN** SHALL render FormField.Root as outer wrapper
- **AND** SHALL render FormField.Label containing label prop
- **AND** SHALL render FormField.Input wrapping TextInput component
- **AND** SHALL pass all TextInput-specific props to TextInput element
- **AND** SHALL pass all FormField-specific props to FormField.Root

#### Scenario: Internal composition
- **WHEN** component renders
- **THEN** TextInput SHALL be wrapped in FormField.Input
...
- **AND** TextInput SHALL receive field state from FormField via props cloning

#### Scenario: Single-component interface
- **WHEN** developer uses TextInputField
- **THEN** SHALL accept flat props API (no nested components required)
- **AND** SHALL eliminate need for manual FormField composition
- **AND** SHALL provide same functionality as FormField + TextInput combination
- **AND** SHALL be drop-in replacement for common text input field patterns
...
- **WHEN** TextInputField is instantiated
...
- **AND** TextInput SHALL receive isRequired via props cloning
- **AND** input SHALL have aria-required="true"

#### Scenario: Disabled state
- **WHEN** isDisabled prop is set
- **THEN** SHALL pass isDisabled to FormField.Root
- **AND** TextInput SHALL receive isDisabled and be non-interactive
...
- **AND** TextInput SHALL receive isReadOnly
- **AND** input SHALL be focusable but non-editable
- **AND** input SHALL have readonly attribute

### Requirement: TextInput Props Passthrough
The component SHALL accept and forward all TextInput props per nimbus-core standards.

#### Scenario: Value and onChange
- **WHEN** value and onChange props are provided
- **THEN** SHALL pass to TextInput for controlled component pattern
- **AND** onChange SHALL receive string value (not event)
- **AND** TextInput SHALL update value on user input
- **AND** controlled pattern SHALL work as expected

#### Scenario: Input type
- **WHEN** type prop is provided
- **THEN** SHALL accept: "text" | "email" | "password" | "search"
- **AND** default type SHALL be "text"
- **AND** SHALL pass type to TextInput
- **AND** TextInput SHALL set HTML input type attribute

#### Scenario: Placeholder
- **WHEN** placeholder prop is provided
- **THEN** SHALL pass to TextInput
- **AND** TextInput SHALL display placeholder in empty input
...
- **THEN** SHALL pass to TextInput
- **AND** TextInput SHALL render decorative elements inside input container
- **AND** elements SHALL support icons, text, or custom components

#### Scenario: Size prop
- **WHEN** size prop is provided
- **THEN** SHALL pass to both FormField.Root and TextInput
- **AND** SHALL accept "sm" | "md" values
- **AND** default size SHALL be "md"
- **AND** FormField typography and TextInput styling SHALL match size

#### Scenario: Style props
- **WHEN** style props (width, maxWidth, margin, etc.) are provided
- **THEN** SHALL pass to TextInput element
...
#### Scenario: Other TextInput props
- **WHEN** additional TextInput props are spread via rest
- **THEN** SHALL pass name, id, defaultValue, autoComplete, maxLength, etc.
- **AND** SHALL support all valid TextInput props
...
- **WHEN** TextInputFieldProps is defined
- **THEN** SHALL extend Omit<TextInputProps, "type">
- **AND** SHALL pick specific FormFieldProps: isRequired, isInvalid, isDisabled, isReadOnly, id
- **AND** SHALL define own type prop with limited values: "email" | "password" | "search" | "text"
- **AND** SHALL export TextInputFieldProps interface
...
- **AND** SHALL match FormField and TextInput size values
...
- **THEN** SHALL set displayName = "TextInputField"
...
- **AND** focus indicator SHALL be provided by TextInput component
...
- **WHEN** TextInputField is inside <form>
...
#### Scenario: Form reset
- **WHEN** form is reset
- **THEN** TextInput SHALL clear to defaultValue or empty
...
- **AND** FormField and TextInput SHALL apply appropriate sizes at breakpoints
...
- **AND** TextInput SHALL apply responsive styles at breakpoints
- **AND** SHALL use Chakra breakpoint system

### Requirement: UI-Kit Compatibility
The component SHALL maintain backwards compatibility with UI-Kit TextField per nimbus-core standards.

#### Scenario: Error format compatibility
- **WHEN** errors prop is provided
- **THEN** SHALL accept same error format as UI-Kit TextField
...
#### Scenario: API parity
- **WHEN** migrating from UI-Kit TextField
- **THEN** TextInputField SHALL support equivalent props
...
The component SHALL provide simpler API than manual FormField + TextInput composition per nimbus-core standards.
...
#### Scenario: When to use TextInputField
- **WHEN** use case is standard text input with label/description/errors
- **THEN** TextInputField SHALL be preferred over manual composition
...
- **THEN** developers SHALL use FormField + TextInput directly
- **AND** manual composition SHALL provide full flexibility
- **AND** TextInputField SHALL not constrain advanced use cases
...
- **AND** SHALL describe purpose: pre-composed field combining TextInput with FormField
