...
The DraggableList component provides accessible drag-and-drop reordering
functionality for list items with keyboard and mouse/touch interactions. Built
with React Aria's GridList for WCAG-compliant accessibility patterns.

**Component:** `DraggableList` (compound namespace) **Package:**
`@commercetools/nimbus` **Type:** Compound component (multi-slot recipe) **React
Aria:** Uses GridList with drag-and-drop hooks for accessibility

## Purpose

The DraggableList component provides an accessible, flexible system for
rendering lists with drag-and-drop reordering capabilities. It supports keyboard
navigation, mouse/touch interactions, controlled/uncontrolled modes, item
removal, and integrates with form libraries through a Field wrapper. The
component follows WCAG 2.1 AA accessibility standards through React Aria's
GridList and drag-and-drop patterns.

## Requirements

### Requirement: Namespace Structure

The component SHALL export as compound component namespace per nimbus-core
standards.

#### Scenario: Component parts

- **WHEN** DraggableList is imported
- **THEN** SHALL provide DraggableList.Root as list container with drag-and-drop
  functionality
...
The component SHALL use React Aria's GridList for accessible list structure and
drag-and-drop.
...
- **AND** SHALL use React Aria's useDragAndDrop hook for drag-and-drop
  functionality
...
The component SHALL support mouse, keyboard, and touch drag-and-drop
interactions.
...
- **THEN** SHALL display DropIndicator component between potential drop
  positions
...
- **AND** CRITICAL: getKey SHALL be memoized with useCallback to prevent re-sync
  loops
...
- **THEN** SHALL throw error with message: 'Nimbus DraggableList: when
  "children" is not provided, items must have both "key" and "label" properties'
...
- **WHEN** list renders
- **THEN** SHALL apply draggableListSlotRecipe from
  theme/slot-recipes/draggable-list.ts
...
- **AND** SHALL apply padding: "200", gap: "200", border: "25 solid",
  borderColor: "colorPalette.3", borderRadius: "200"
...
- **AND** root SHALL apply italic font style and colorPalette.11 color when
  data-empty
...
- **WHEN** DraggableList.Field is used
- **THEN** items SHALL have both key and label properties (type:
...
- **THEN** SHALL export DraggableListRootProps interface with generic type
  parameter
- **AND** SHALL export DraggableListItemProps interface with generic type
  parameter
- **AND** SHALL export DraggableListFieldProps interface with generic type
  parameter
...
- **THEN** SHALL export DraggableListRecipeProps with auto-generated variant
  types
...
- **AND** SHALL export DraggableListFieldItemData type with required key and
  label
- **AND** SHALL support generic Record<string, unknown> for additional
  properties

#### Scenario: Slot props types

- **WHEN** slots are typed
- **THEN** SHALL export DraggableListRootSlotProps, DraggableListItemSlotProps,
  DraggableListItemContentSlotProps, DraggableListEmptySlotProps
- **AND** SHALL combine HTMLChakraProps with component-specific props

### Requirement: Optimized State Management

The component SHALL optimize internal state synchronization to prevent render
loops.
...
- **AND** SHALL use queueMicrotask to reset sync flag after state updates
  complete
...
- **AND** ArrowRight SHALL navigate between item cells (checkbox, drag handle,
...
The component SHALL provide comprehensive screen reader support per nimbus-core
accessibility standards.
...
The component SHALL meet touch target size requirements per nimbus-core
accessibility standards.
...
- **THEN** SHALL return array of items with "nimbus-draggable-list-item" data
  format
...
- **AND** SHALL prioritize correctness over incremental updates for typical use
  cases
