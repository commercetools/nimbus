...
The FieldErrors component provides standardized error message rendering for form validation scenarios. It translates error objects into user-facing error messages with built-in internationalization support, custom message overrides, and flexible rendering strategies.

**Component:** `FieldErrors`
**Package:** `@commercetools/nimbus`
**Type:** Utility component (error message renderer)
**React Aria:** None (presentational component)
**Pattern:** Error object to message rendering with i18n

## Purpose

Provide a reusable error message rendering component that handles common validation errors with built-in localized messages while supporting custom error messages and renderers. Ensures consistent error presentation across all form inputs in the Nimbus design system while maintaining backwards compatibility with UI-Kit patterns.
...
The component SHALL provide 15 standardized error types with localized messages per nimbus-core standards.
...
- **AND** constants SHALL map to string keys (MISSING → "missing", MIN_LENGTH → "min", etc.)
...
The component SHALL support custom messages for built-in error types via customMessages prop.
...
- **AND** SHALL use custom message even if renderError and renderDefaultError are not provided
...
- **AND** SHALL support FormattedMessage components with values
...
- **AND** SHALL render nothing if renderError returns null and no other renderer handles key

### Requirement: Default Error Renderer
The component SHALL support fallback error rendering via renderDefaultError prop.
...
- **THEN** rendering SHALL follow priority: renderError → renderDefaultError → customMessages → built-in messages
...
The component SHALL support visibility control via isVisible prop per nimbus-core standards.
...
- **THEN** SHALL use fieldErrors slot recipe from theme/slot-recipes/field-errors.ts
...
#### Scenario: FormattedMessage rendering
- **WHEN** built-in messages render
- **THEN** SHALL use useIntl hook or FormattedMessage component
- **AND** SHALL format messages according to current locale
...
The component SHALL follow a defined priority order for error message resolution.
