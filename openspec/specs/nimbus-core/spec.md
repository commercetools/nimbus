...
**Package:** `@commercetools/nimbus` **Version:** 2.1.0

## Purpose

Establish cross-cutting requirements and standards that ALL components in the
@commercetools/nimbus package MUST adhere to, ensuring consistency,
accessibility, type safety, and maintainability across the entire design system.
Individual component capabilities are documented in separate component-specific
specs.
...
- **AND** SHALL support component-specific keys (Arrow keys, Enter, Escape,
  Space, Home, End)
...
- **THEN** components SHALL provide appropriate ARIA attributes (role,
...
- **AND** interactive elements SHALL be distinguishable without relying on color
  alone
...
All interactive components SHALL use React Aria Components for accessibility
primitives.
...
- **THEN** SHALL export {Component}RecipeProps (single-slot) or
  {Component}SlotRecipeProps (multi-slot)
...
- **AND** SHALL externalize peer dependencies (React, Chakra, Slate, plain
  TypeScript objects)
...
- **AND** SHALL require @commercetools/nimbus-icons and
...
- **THEN** SHALL export as namespace object (e.g., Menu.Root, Menu.Trigger,
  Menu.Item)
...
- **AND** SHALL include: index.ts (exports), {component}.tsx (implementation),
  {component}.types.ts (types), {component}.stories.tsx (tests)
- **AND** MAY include: {component}.recipe.ts, {component}.slots.tsx,
  {component}.i18n.ts, {component}.mdx
...
- **THEN** SHALL import from implementation file:
  `from '@/components/icon/icon'`
- **AND** SHALL NOT import from barrel export (index.ts) to avoid circular chunk
  dependencies
