...
- [ ] 1.1 Rename directory `packages/nimbus/src/components/chat-bubble/` →
...
- [ ] 3.1 Create `ChatNotice` following the Nimbus file layout (a single-slot
...
- [ ] 3.2 Type `ChatNoticeProps` (children + style props + `data-*`); set
      `displayName`.
- [ ] 3.3 Export `ChatNotice` and its types from the package barrel.
- [ ] 3.4 Register the `ChatNotice` recipe in theme config.

## 4. Streaming delegation

- [ ] 4.1 Keep `isStreaming` on `ChatMessage.Root` setting `aria-busy` only.
      ...
      content.
- [ ] 4.2 Keep `ChatMessage.Typing` (the `ActivityIndicator` dots +
      optional label) as the pre-first-token affordance.
- [ ] 4.3 Ensure `ChatMessage.Root` does **not** create its own `aria-live`
      region (announcement is the transcript/consumer container's job).

## 5. Figma Code Connect

- [ ] 5.1 Rename `chat-message.figma.tsx`; map the Figma `Sender` property
      (`User`/`Agent`) onto the `sender` variant; forward children into
      `ChatMessage.Bubble`.

## 6. Stories & tests (Storybook play functions)

- [ ] 6.1 Update stories to `ChatMessage`: base, user, agent, actions, meta,
      markdown payload, overflow/wrapping, error tone, streaming (via
      `<Markdown isStreaming>` + `isStreaming` busy flag).
- [ ] 6.2 Add a `ChatNotice` story (centered notice / divider) replacing the old
      `SystemMessage` story.
- [ ] 6.3 Replace the old `ToolMessage` story with an `agent` message whose
      bubble content is tool output (code block via `Markdown`).
- [ ] 6.4 Keep/extend a11y assertions: article-by-default, named message,
      decorative-vs-named avatar, `aria-busy` while streaming.

## 7. Documentation

- [ ] 7.1 Update `chat-message.mdx`, `.dev.mdx`, `.a11y.mdx`, `.docs.spec.tsx`:
...
- [ ] 7.2 Update the lifecycle/description front-matter (still Experimental).

## 8. Spec housekeeping

- [ ] 8.1 On archive, this change ADDs capability `nimbus-chat-message` and
      REMOVEs `nimbus-chat-bubble` (see the change's `specs/` deltas).

## 9. Verification

- [ ] 9.1 `pnpm --filter @commercetools/nimbus typecheck:dev` clean.
- [ ] 9.2 `pnpm test:dev packages/nimbus/src/components/chat-message/…` green
      (stories + play functions).
- [ ] 9.3 `pnpm lint` clean; grep the repo for stale `ChatBubble` /
