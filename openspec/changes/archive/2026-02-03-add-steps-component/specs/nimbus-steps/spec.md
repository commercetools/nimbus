...
The Steps component provides a display-only progress indicator for multi-step
processes like forms, wizards, and onboarding flows. It enables users to see
their current position and progress through sequential tasks with clear visual
states and full WCAG 2.1 AA compliance.

**Component:** `Steps` (compound namespace) **Package:** `@commercetools/nimbus`
**Type:** Compound component (multi-slot recipe) **React Aria:** Custom
implementation using React Aria patterns (no primitives available)
