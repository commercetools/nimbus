...
> Note: Implementation chose React 19 automatic link hoisting via an internal
> `InterFontLoader` component inside `nimbus-provider.tsx` instead of a
> separate `use-font-loader.ts` hook. Tasks below are ticked against the
> shipped approach; hook-specific subtasks are left unchecked as they were
> superseded by the simpler, React 19-native implementation.

## 1. Type Definitions

- [x] 1.1 Add `loadFonts?: boolean` prop to `NimbusProviderProps` interface in `nimbus-provider.types.ts`
- [x] 1.2 Add JSDoc comment: "Load Inter font from Google Fonts. Set to false if fonts are loaded by host application. @default true"
- [x] 1.3 Verify types build correctly with `pnpm --filter @commercetools/nimbus typecheck`
...
- [x] 2.6 Inject preconnect link for `https://fonts.googleapis.com`
- [x] 2.7 Inject preconnect link for `https://fonts.gstatic.com` with `crossorigin` attribute
- [x] 2.8 Inject stylesheet link with Google Fonts CSS API v2 URL: `https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap`
- [x] 2.9 Add `data-nimbus-fonts` attribute to stylesheet link for identification
...
- [x] 2.12 Add TypeScript types and JSDoc documentation

## 3. Provider Integration

- [x] 3.1 Import `useFontLoader` hook in `nimbus-provider.tsx`
- [x] 3.2 Destructure `loadFonts = true` from props with default value
- [x] 3.3 Call `useFontLoader(loadFonts)` at top of component
- [x] 3.4 Ensure hook runs before all other effects
- [x] 3.5 Verify no changes needed to existing provider logic
- [x] 3.6 Add component-level comment explaining font loading behavior

## 4. Unit Tests

- [ ] 4.1 Create `use-font-loader.spec.ts` in hooks directory
- [ ] 4.2 Mock `document.head.append` and `document.head.querySelector`
- [x] 4.3 Test: Font links injected when enabled=true
- [x] 4.4 Test: No injection when enabled=false
...
- [x] 4.8 Test: Link attributes are correct (href, rel, crossorigin, data-nimbus-fonts)
- [ ] 4.9 Run tests: `pnpm test use-font-loader.spec.ts`
- [x] 4.10 Ensure all tests pass

## 5. Storybook Stories

- [x] 5.1 Add story in `nimbus-provider.stories.tsx` demonstrating font loading
- [x] 5.2 Add story showing `loadFonts={false}` opt-out
- [x] 5.3 Add play function to verify font links injected in DOM
- [x] 5.4 Add visual test showing Inter font rendering
...
- [x] 6.1 Update `nimbus-provider.dev.mdx` with font loading section
- [x] 6.2 Document default behavior (fonts load automatically)
- [x] 6.3 Document opt-out scenario (Merchant Center example)
- [x] 6.4 Document fallback behavior when Google Fonts unavailable
- [x] 6.5 Add code examples for both default and opt-out usage
- [x] 6.6 Update component JSDoc comment in `nimbus-provider.tsx` to mention font loading
- [x] 6.7 Create migration guide section for Merchant Center integration
