# Specification: ChatBubble

## Overview

The ChatBubble component lays out and styles a single message in an AI chat /
assistant feed. It is a compound component: a `ChatBubble.Root` grid container
plus composable, layout-only parts the consumer fills with their own content —
an avatar, a bubble card, an actions row, a footer row, and a typing indicator.

`sender` selects the layout direction and per-sender surface; `tone` is an
orthogonal status overlay (e.g. a failed generation). ChatBubble renders one
message only — the surrounding feed and any live region are composed by the
consumer — so it stays flexible across the different chat UIs teams build. It
ships accessible defaults (a semantic `<article>`, a decorative avatar) with
escape hatches, and documents the recommended feed/live-region composition
rather than forcing it.

**Component:** `ChatBubble` **Package:** `@commercetools/nimbus` **Category:**
Feedback
