...
- [ ] 1.2 Add `export * from "./confirmation-dialog"` to
      `packages/nimbus/src/patterns/dialogs/index.ts`
      (alongside the existing `info-dialog` export)
- [ ] 1.3 Create `confirmation-dialog/index.ts` barrel re-exporting
      the component and types

## 2. i18n

- [ ] 2.1 Implement `confirmation-dialog.i18n.ts` exporting a
...
- [ ] 3.1 Implement `confirmation-dialog.types.ts` with
      ...
      every prop.
- [ ] 3.2 Implement `confirmation-dialog.tsx` composing
...
- [ ] 3.3 In `confirmation-dialog.tsx`, resolve default labels via
      `useLocalizedStringFormatter(messages)` from `@/hooks`,
      following the FormActionBar precedent
- [ ] 3.4 In `confirmation-dialog.tsx`, render a
...
- [ ] 3.5 In `confirmation-dialog.tsx`, wire `onOpenChange` so
...
- [ ] 3.6 In `confirmation-dialog.tsx`, wire the confirm `Button`
      `onPress` to invoke `onConfirm` and then propagate
      `onOpenChange(false)` to the consumer
- [ ] 3.7 Set `ConfirmationDialog.displayName = "ConfirmationDialog"`

## 4. Testing

- [ ] 4.1 Write Storybook stories
...
- [ ] 5.1 Create `confirmation-dialog.dev.mdx` with: import
...
- [ ] 5.2 In `confirmation-dialog.dev.mdx`, include a "migrating
      ...
      hatch)
- [ ] 5.3 Create `confirmation-dialog.mdx` with frontmatter
      `related-components: [Dialog, Button]` and
      `menu: [Patterns, Dialogs, Confirmation dialog]`
- [ ] 5.4 Verify the generated PropsTable correctly reflects the
...
- [ ] 6.3 Storybook stories render with all play functions passing
      (`pnpm test:storybook`)
- [ ] 6.4 Consumer implementation tests pass (`pnpm test`)
- [ ] 6.5 `openspec validate confirmation-dialog-pattern --strict`
      passes
