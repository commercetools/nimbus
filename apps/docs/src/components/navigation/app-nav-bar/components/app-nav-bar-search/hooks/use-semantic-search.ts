...
import type { WorkerRequest, WorkerResponse } from "../semantic-search.types";
 ...
 * @param docs    - The documents to index (the search index entries).
 * @param query   - The (debounced) query to run once ready; "" clears results.
 */
export function useSemanticSearch(
  enabled: boolean,
  docs: SearchableDocItem[],
  query: string
): SemanticSearchState {
  ...
  const [results, setResults] = useState<SearchableDocItem[]>([]);
  ...
  useEffect(() => {
        ...
        case "results":
          ...
          setResults(
            message.ids
              ...
              .filter((doc): doc is SearchableDocItem => doc !== undefined)
          );
          break;
        case "error":
          setStatus("error");
    ...
    const init: WorkerRequest = { type: "init", docs };
