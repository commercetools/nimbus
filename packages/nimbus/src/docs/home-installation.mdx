...
For proper TypeScript autocomplete and type checking, generate types for Nimbus's custom recipes:

```bash
# Create a theme export file
echo 'export { system } from "@commercetools/nimbus"' > src/theme.ts

# Generate TypeScript types
pnpm dlx @chakra-ui/cli typegen ./src/theme.ts
```

This command reads Nimbus's theme configuration and generates TypeScript definitions for custom component recipes (Drawer, Alert, Card, etc.).

> [!IMPORTANT]
> Every developer on your team needs to run the typegen command after installing Nimbus. The generated types are written to `node_modules/` and are not committed to version control.

### Optional: Automate with Postinstall

You can add this as a postinstall script to run automatically:

```json
{
  "scripts": {
    "postinstall": "chakra-ui typegen ./src/theme.ts"
  }
}
```

## 4. Use Nimbus Components

Once the provider is set up and types are generated, you can start using Nimbus components in your
application.
