...
**Accordion item header example** → Flexible content ✅:
- Can have simple text: `<Text slot="header">Section 1</Text>`
- Can have rich content: `<Stack slot="header"><Text>Title</Text><Button>Action</Button></Stack>`
- **Why slots win:** Headers often need buttons, badges, or complex layouts
...
2. **Fix existing APIs** to use correct patterns:
   ...
   - **Accordion**: Change from `<Accordion.Header>` to `<Text slot="header">` within `<Accordion.Item>`
