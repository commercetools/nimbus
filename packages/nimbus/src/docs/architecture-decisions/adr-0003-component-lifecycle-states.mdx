...
- **Transitions**: To Beta (if successful) or Deprecated (if unviable)
...
- **Transitions**: To Stable (if successful) or Deprecated (if unviable)
...
#### 5. Deprecated
- **Purpose**: No longer recommended for new development; superseded by
  better alternatives
...
- **Marking**: Clear warnings in documentation with recommended alternatives;
  `@deprecated` JSDoc tag; console warnings during development
