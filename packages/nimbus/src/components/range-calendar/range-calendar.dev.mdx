  ...
  return (
    <Stack direction="column" gap="400">
      <RangeCalendar
        ...
        onChange={(range) => {
          if (range) {
            setSelectedRange(`${formatDate(range.start)} to ${formatDate(range.end)}`);
          } else {
            setSelectedRange('No selection');
          }
        }}
      />
...
```jsx-live-dev
const App = () => {
  const [value, setValue] = useState<RangeCalendarProps["value"]>(null);
