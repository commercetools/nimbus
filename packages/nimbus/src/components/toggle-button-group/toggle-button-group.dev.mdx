...
`ToggleButtonGroup.Button` is the standard [`ToggleButton`](/components/buttons/togglebutton) exposed under the compound name. It inherits the group's `variant`, `activeFillStyle`, `size`, and `colorPalette`, and you can override any of them on a single button (see [Overriding individual buttons](#overriding-individual-buttons)).

## Usage examples

### Size options

The component supports five size variants (`2xs`, `xs`, `sm`, `md`, `xl`) to
match your interface density — the same scale as `Button` and `ToggleButton`:

```jsx live-dev
const App = () => (
  <Stack direction="column" gap="400">
    <ToggleButtonGroup.Root size="2xs" aria-label="2x small text alignment">
      <ToggleButtonGroup.Button id="left">Left</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="center">Center</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="right">Right</ToggleButtonGroup.Button>
    </ToggleButtonGroup.Root>

    <ToggleButtonGroup.Root size="xs" aria-label="Extra small text alignment">
      <ToggleButtonGroup.Button id="left">Left</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="center">Center</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="right">Right</ToggleButtonGroup.Button>
    </ToggleButtonGroup.Root>

    <ToggleButtonGroup.Root size="sm" aria-label="Small text alignment">
      <ToggleButtonGroup.Button id="left">Left</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="center">Center</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="right">Right</ToggleButtonGroup.Button>
    </ToggleButtonGroup.Root>
    ...
    <ToggleButtonGroup.Root size="xl" aria-label="Extra large text alignment">
      <ToggleButtonGroup.Button id="left">Left</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="center">Center</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="right">Right</ToggleButtonGroup.Button>
    </ToggleButtonGroup.Root>
...
`variant` sets the resting chrome shared by every button in the group. The group
renders segmented (buttons sit flush with shared borders) regardless of variant.

- `outline` (default): a neutral segmented bar.
- `subtle`: filled neutral segments.

```jsx live-dev
const App = () => (
  <Stack direction="column" gap="400">
    <ToggleButtonGroup.Root variant="outline" defaultSelectedKeys={["center"]} aria-label="Outline alignment">
      <ToggleButtonGroup.Button id="left">Left</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="center">Center</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="right">Right</ToggleButtonGroup.Button>
    </ToggleButtonGroup.Root>

    <ToggleButtonGroup.Root variant="subtle" defaultSelectedKeys={["center"]} aria-label="Subtle alignment">
      <ToggleButtonGroup.Button id="left">Left</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="center">Center</ToggleButtonGroup.Button>
      <ToggleButtonGroup.Button id="right">Right</ToggleButtonGroup.Button>
    </ToggleButtonGroup.Root>
...
`activeFillStyle` sets the weight of the selected fill for the buttons. Its
default follows `selectionMode`: **single** selection defaults to `solid` (one
choice stands out), **multiple** selection defaults to `tint` (several can be on
at once). Set it explicitly to override that default.

```jsx live-dev
const App = () => (
  <Stack direction="column" gap="400">
    <Stack direction="column" gap="200">
      <Text fontSize="350" fontWeight="600">Single selection → solid (default)</Text>
      <ToggleButtonGroup.Root colorPalette="primary" defaultSelectedKeys={["center"]} aria-label="Solid alignment">
        <ToggleButtonGroup.Button id="left">Left</ToggleButtonGroup.Button>
        <ToggleButtonGroup.Button id="center">Center</ToggleButtonGroup.Button>
        <ToggleButtonGroup.Button id="right">Right</ToggleButtonGroup.Button>
      </ToggleButtonGroup.Root>
    ...
    <Stack direction="column" gap="200">
      <Text fontSize="350" fontWeight="600">Multiple selection → tint (default)</Text>
      <ToggleButtonGroup.Root selectionMode="multiple" colorPalette="primary" defaultSelectedKeys={["bold", "italic"]} aria-label="Tint formatting">
        <ToggleButtonGroup.Button id="bold">Bold</ToggleButtonGroup.Button>
        <ToggleButtonGroup.Button id="italic">Italic</ToggleButtonGroup.Button>
        <ToggleButtonGroup.Button id="underline">Underline</ToggleButtonGroup.Button>
      </ToggleButtonGroup.Root>
    </Stack>
  </Stack>
)
...
Choose from semantic color palettes to match your design context. As with a
standalone toggle button, the resting group is neutral — `colorPalette` tints the
...
Because each `ToggleButtonGroup.Button` is a full `ToggleButton`, any prop set on
a single button overrides the value inherited from the group. This is useful for
drawing attention to a destructive or primary option within an otherwise uniform
group:

```jsx live-dev
const App = () => (
  <ToggleButtonGroup.Root colorPalette="primary" defaultSelectedKeys={["keep"]} aria-label="Row action">
