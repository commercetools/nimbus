...
The DatePicker component relies on `@internationalized/date`'s date types for type-safe date handling. This library provides calendar-system-aware date representations that work correctly across different locales and time zones.
...
- [Calendar Component](/components/date/calendar)
