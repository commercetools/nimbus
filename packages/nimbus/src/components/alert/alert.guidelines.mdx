...
- **User experience:** Place alerts strategically, allow dismissal when
  ...
  - **Flat (`variant="flat"`):** No chrome. Use inline in forms and within
    ...
    status.
  - **Outlined (`variant="outlined"`):** Tinted card. Be mindful of
    how many alerts occur on one page. These are best used for page or product
    wide information. They are not meant to be used as toasts.
  - **Accent start (`variant="accent-start"`):** Neutral card with a
    status-colored bar on the leading edge — close to the treatment Toast uses. The
    surface stays neutral, so several can share a page without it reading as a
    color chart. Use it where the message matters but the layout should stay
    calm.
- **Layout:** Alert has one arrangement and no `layout` prop — the icon leads,
  the title and description stack beside it, actions sit below, and the
  dismiss button sits on the trailing edge.
  - For a **page-level banner**, use the same alert with its corners and side
    borders removed (`borderRadius="0" borderInline="none"`) rather than a
    dedicated layout.
  - For a **compact horizontal row** with leading media and trailing actions —
    a settings row, a notification row, an agent confirmation with an "Undo" —
    use `Item`. Alert is for status messages that may need announcing.
- **Accessibility and design:** Maintain consistent styling, use color and icons
  ...
  - **Announcement (polite vs. assertive):** Critical alerts default to an
    assertive, interrupting announcement (`role="alert"`); every other palette
    defaults to a polite one (`role="status"`) that doesn't interrupt the user.
    Keep the assertive announcement for errors that truly require immediate
    attention — using it for routine messages contributes to alert fatigue and
    undermines its urgency when it matters most.
