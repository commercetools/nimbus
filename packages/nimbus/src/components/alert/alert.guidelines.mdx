...
- **User experience:** Place alerts strategically, allow dismissal when
  ...
  information.
  - **Flat (`variant="flat"`):** No chrome. Use inline in forms and within
    popovers when needed. These can be used inside forms to signal validation
    status.
  - **Subtle (`variant="subtle"`, default):** Tinted card. Be mindful of how
    many alerts occur on one page. These are best used for page or product
    wide information. They are not meant to be used as toasts.
    (`variant="outlined"` is a deprecated alias for `subtle` — prefer
    `subtle` going forward.)
  - **Outline (`variant="outline"`):** Border-only, minimal fill. A
    lighter-weight alternative to `subtle` for the same page/product-wide
    contexts.
  - **Solid (`variant="solid"`):** High-contrast fill. Reserve for the
    messages that most need to stand out, such as page-level banners — use
    the sparsest.
- **Layout:** Choose `layout` to match how the alert sits in the page.
  - **`stack` (default):** icon and content stack vertically. The general-purpose
    layout for most standalone alerts.
  - **`inline`:** a compact, single-line row with the action trailing —
    content leading, actions/dismiss trailing, wrapping on narrow widths. Use
    for section-scoped confirmations, such as an agent-confirmation banner
    with an "Undo" action.
  - **`banner`:** full-width, edge-to-edge, no border radius. Use for
    page-level messages that span the full width of the page.
- **Accessibility and design:** Maintain consistent styling, use color and icons
  ...
  - **Announcement (polite vs. assertive):** Alerts default to a polite
    announcement (`role="status"`) that doesn't interrupt the user. Reserve
    the assertive, interrupting announcement (`role="alert"`) for critical
    errors that truly require immediate attention — using it for routine
    messages contributes to alert fatigue and undermines its urgency when it
    matters most.
...
const App = () => (
  <Box>
   ...
   <Alert.Root colorPalette="critical" variant="subtle" width="9600" position="relative" top="-500" left="500">
