...
alerts within our design system, covering different visual treatments and
severities for diverse use cases.
...
The `variant` prop controls emphasis. Without a `variant`, the alert has no surface and no padding and sits flush with its container.
...
A neutral card with a status-colored bar down the leading edge — close to the
treatment `Toast` uses by default. The surface and the text stay neutral and
the bar, the icon and the action buttons carry the status color, so a page can hold several of
these at once without turning into a color chart. Use it where the message
matters but the surrounding layout should stay calm. The bar follows the
reading direction and moves to the right edge in right-to-left languages.

```jsx live
const App = () => (
  <Alert.Root colorPalette="warning" variant="accent-start" width="8000">
    <Alert.Title>Inventory Is Running Low</Alert.Title>
    <Alert.Description>
      Three products in this category have fewer than ten units left in stock.
    </Alert.Description>
  </Alert.Root>
)
```
...
Alert has one layout: the status icon on the leading edge, the title and
description stacked beside it, actions below, and the dismiss button on the
trailing edge. There is no `layout` prop — anything beyond this arrangement is
a matter of placement and style props.

A full-width page banner, for example, is the same alert with its corners and
...
const App = () => (
  <Stack direction="column" gap="400">
    <Alert.Root colorPalette="info" variant="outlined">
      ...
      <Alert.Description>
        Icon on the leading edge, title and description beside it, actions and
        the dismiss button below.
      </Alert.Description>
    </Alert.Root>
...
For a compact horizontal row with leading media and trailing actions — a
settings row, a notification row, an agent confirmation — reach for `Item`
instead. Alert is for status messages that may need announcing.
...
immediately. Because these must interrupt the user, `critical` defaults to
`role="alert"`; every other palette defaults to the polite `"status"` (see the
...
```jsx live
const App = () => (
  <Alert.Root colorPalette="neutral" variant="outlined" width="8000" hideIcon>
