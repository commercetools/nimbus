...
The `colorPalette` prop determines the semantic color and the accompanying icon.
...
- All sub-components (`Title`, `Description`, `Actions`, `DismissButton`) **must** be direct children of `Alert.Root` to function correctly. They communicate via context to register themselves for layout.
- Do not wrap sub-components in other elements (like `div` or `Stack`) if you want them to be automatically placed in the correct slot of the alert grid. You *can* use layout components *inside* `Alert.Actions` or `Alert.Description`.
