...
const App = () => (
  <Stack direction="column" gap="400">
    <TimeInput
      leadingElement={<Icon as={Clock} />}
      aria-label="Time with leading icon"
      defaultValue={new Time(9, 0)}
    />
    <TimeInput
      trailingElement={<Icon as={Schedule} />}
  ...
  return (
    <Stack direction="column" gap="400">
      ...
      <Stack direction="row" gap="300">
        <Button size="sm" onPress={() => setValue(new Time(9, 0))}>
          Set to 9:00 AM
        </Button>
        <Button size="sm" onPress={() => setValue(new Time(17, 0))}>
          Set to 5:00 PM
        </Button>
        <Button size="sm" variant="secondary" onPress={() => setValue(null)}>
          Clear
        </Button>
      </Stack>
      <Text fontSize="sm">
...
const App = () => {
  ...
  return (
    <Stack direction="column" gap="600">
      ...
      <Stack direction="column" gap="200">
        <Text fontSize="sm" fontWeight="semibold">French (fr-FR)</Text>
        <NimbusI18nProvider locale="fr-FR">
          <TimeInput
            aria-label="French locale"
            defaultValue={sampleTime}
          />
        </NimbusI18nProvider>
      </Stack>
      <Stack direction="column" gap="200">
        <Text fontSize="sm" fontWeight="semibold">Japanese (ja-JP)</Text>
        <NimbusI18nProvider locale="ja-JP">
          <TimeInput
            aria-label="Japanese locale"
            defaultValue={sampleTime}
          />
        </NimbusI18nProvider>
      </Stack>
...
### Meeting scheduler with 30-minute increments

Guide users to select times in specific increments:
...
const App = () => {
  ...
  const handleTimeChange = (newTime: TimeValue | null) => {
    if (newTime && newTime.minute % 30 !== 0) {
      // Round to nearest 30-minute increment
      const roundedMinute = Math.round(newTime.minute / 30) * 30;
      const adjustedTime = newTime.set({ minute: roundedMinute % 60 });
  ...
  return (
    <Stack direction="column" gap="400">
      ...
      />
      <Text fontSize="sm" color="neutral.11">
        Times are automatically rounded to 30-minute increments
      </Text>
      {time && (
        <Text fontSize="sm">
          Selected slot: {time.toString().slice(0, 5)}
        </Text>
      )}
    </Stack>
  );
