...
The example above uses `IconToggleButton` children, so each button keeps the
toolbar's normal spacing. When you use `ToggleButtonGroup.Button` children the
group is **segmented** (buttons share borders), and the toolbar renders those
segments flush — no gap between them — while the group as a whole keeps the
toolbar spacing from its neighbours:

```jsx live-dev
const App = () => (
  <Toolbar variant="outline">
    <ToggleButtonGroup.Root defaultSelectedKeys={["left"]} aria-label="Alignment">
