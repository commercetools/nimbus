...
const App = () => (
  <Toolbar>
    <Button size="xs" variant="ghost">File</Button>
...
These examples demonstrate how to test your implementation when using Toolbar in your application. The component's internal functionality is already tested by Nimbus - these patterns help you verify your integration and application-specific logic.
...
import { Toolbar, Button } from '@commercetools/nimbus';
...
describe('Toolbar keyboard navigation', () => {
  it('navigates between controls with arrow keys in horizontal toolbar', async () => {
    ...
    render(
      <Toolbar aria-label="Test toolbar">
        <Button size="xs" data-testid="btn-1">File</Button>
        <Button size="xs" data-testid="btn-2">Edit</Button>
        <Button size="xs" data-testid="btn-3">View</Button>
      </Toolbar>
    );
  ...
  it('navigates between controls with arrow keys in vertical toolbar', async () => {
    ...
    render(
      <Toolbar orientation="vertical" aria-label="Test toolbar">
        <Button size="xs" data-testid="btn-1">Home</Button>
        <Button size="xs" data-testid="btn-2">Search</Button>
        <Button size="xs" data-testid="btn-3">Settings</Button>
      </Toolbar>
    );
...
### Testing with ToggleButtonGroup

Test toolbar integration with toggle button groups:

```tsx
import { render, screen } from '@testing-library/react';
import userEvent from '@testing-library/user-event';
import { Toolbar, ToggleButtonGroup, IconToggleButton } from '@commercetools/nimbus';

describe('Toolbar with ToggleButtonGroup', () => {
  it('maintains toggle states within toolbar', async () => {
    const user = userEvent.setup();
    render(
    ...
    const italicBtn = screen.getByTestId('italic-btn');
    ...
    // Click italic to activate it
    await user.click(italicBtn);
    expect(italicBtn).toHaveAttribute('aria-pressed', 'true');

    // Bold should still be active (multiple selection mode)
    expect(boldBtn).toHaveAttribute('aria-pressed', 'true');

    // Click bold to deactivate it
