...
Use `trailingElement` to add content after the selected value, before the clear
button and chevron. It renders beside the trigger button rather than inside it,
so interactive content is valid here and keeps its own behaviour — pressing a
trailing button does not open the listbox:

```jsx live-dev
const App = () => (
  <Select.Root
    leadingElement={<Icon as={Icons.Search} />}
    trailingElement={
      <IconButton
        size="xs"
        variant="ghost"
...
When space runs short the selected value truncates with an ellipsis rather than
running underneath the trailing element. Mark decorative content
`aria-hidden="true"`; interactive content needs its own `aria-label`.

Keep the trigger readable — a leading icon, a trailing control, the clear button
and the chevron can all appear at once, so add only what earns its place.
