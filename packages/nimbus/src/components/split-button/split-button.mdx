...
The SplitButton component addresses the need for interfaces that require a dominant action with several less frequently used, but still important, related options. It supports two modes: split button mode (with a separate primary action button) and regular button mode (entire button opens dropdown).
...
When no `defaultAction` is specified, the entire button opens the dropdown.

```jsx-live
const App = () => (
  <SplitButton onAction={(actionId) => alert(`Action: ${actionId}`)}>
...
const App = () => (
  <Stack direction="horizontal" gap="400" alignItems="center">
    <SplitButton size="2xs" defaultAction="option1" onAction={(id) => {}}>
      <Menu.Item id="option1">Option 1</Menu.Item>
      <Menu.Item id="option2">Option 2</Menu.Item>
    </SplitButton>
    <SplitButton size="xs" defaultAction="option1" onAction={(id) => {}}>
      <Menu.Item id="option1">Option 1</Menu.Item>
      <Menu.Item id="option2">Option 2</Menu.Item>
    </SplitButton>
    <SplitButton size="md" defaultAction="option1" onAction={(id) => {}}>
      <Menu.Item id="option1">Option 1</Menu.Item>
      <Menu.Item id="option2">Option 2</Menu.Item>
    </SplitButton>
  </Stack>
...
const App = () => (
  <Stack direction="horizontal" gap="400" wrap="wrap">
    <SplitButton variant="solid" tone="primary" defaultAction="action1" onAction={(id) => {}}>
      <Menu.Item id="action1">Action 1</Menu.Item>
      <Menu.Item id="action2">Action 2</Menu.Item>
    </SplitButton>
    <SplitButton variant="outline" tone="primary" defaultAction="action1" onAction={(id) => {}}>
      <Menu.Item id="action1">Action 1</Menu.Item>
      <Menu.Item id="action2">Action 2</Menu.Item>
    </SplitButton>
    <SplitButton variant="ghost" tone="primary" defaultAction="action1" onAction={(id) => {}}>
      <Menu.Item id="action1">Action 1</Menu.Item>
      <Menu.Item id="action2">Action 2</Menu.Item>
    </SplitButton>
  </Stack>
...
const App = () => (
  <Stack direction="horizontal" gap="400">
    <SplitButton defaultAction="available" onAction={(id) => {}}>
    ...
    <SplitButton isDisabled defaultAction="action1" onAction={(id) => {}}>
...
The SplitButton component has two modes:

### Split Button Mode (with `defaultAction`)
1. **Primary Button**: Executes the specified default action directly
2. **Dropdown Trigger**: Separate arrow button opens the dropdown menu
3. **Menu Actions**: Selecting items from the dropdown triggers the `onAction` callback

### Regular Button Mode (no `defaultAction`)
1. **Single Button**: The entire button opens the dropdown menu
2. **Menu Selection**: Users select from the dropdown to trigger actions
3. **Space Efficient**: Useful when all actions are equally important
...
- **Clear Primary Action**: When using split mode, ensure the default action is the most common or important
...
- **Dual Modes**: Split button mode with primary action, or regular button mode
...
- **Flexible Content**: Supports icons, labels, and rich menu content through slots
...
const App = () => (
  <SplitButton
    defaultAction="send"
    aria-label="Send message with additional options"
    onAction={(actionId) => alert(`Action: ${actionId}`)}
  >
