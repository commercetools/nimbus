---
title: MultilineTextInput Component
  ...
  return (
    <Stack direction="column" gap="400">
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center">
        ...
        <Button size="sm" isDisabled={wordCount < 10}>
          Submit Feedback
        </Button>
      </Stack>
    </Stack>
...
These examples demonstrate how to test your implementation when using MultilineTextInput in your application. The component's internal functionality is already tested by Nimbus - these patterns help you verify your integration and application-specific logic.
...
describe('MultilineTextInput', () => {
  it('renders textarea element', () => {
    render(<MultilineTextInput aria-label="Description" />);
  ...
  it('renders with specified number of rows', () => {
    render(<MultilineTextInput rows={5} aria-label="Description" />);
...
describe('MultilineTextInput interactions', () => {
  it('handles typing multi-line text', async () => {
    ...
    render(<MultilineTextInput aria-label="Message" />);
...
### Testing auto-grow functionality

When using the `autoGrow` feature, test height adjustments:
...
describe('MultilineTextInput auto-grow', () => {
  it('expands height when content increases', async () => {
    const user = userEvent.setup();
    render(
      <MultilineTextInput
        autoGrow
  ...
  it('shrinks height when content decreases', async () => {
    const user = userEvent.setup();
    render(
      <MultilineTextInput
        autoGrow
    ...
    expect(textarea.clientHeight).toBeLessThan(initialHeight);
  });
});
```
...
describe('MultilineTextInput with character limit', () => {
      ...
      return (
        <>
          <MultilineTextInput
            ...
            aria-label="Comment"
          />
          <div data-testid="char-count">{value.length}/{maxLength}</div>
        </>
      );
    };
