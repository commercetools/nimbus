...
title: ComboBox
description:
  A combo box combines a text input with a dropdown list, allowing users to
...
const App = () => {
  const options = [
    ...
    { id: "toys-only", name: "Toys only", description: "Balls, plush toys, and tug toys" },
...
const App = () => {
  ...
  const isLoading = isPending || list.loadingState === "loading";
