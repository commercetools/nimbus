...
Add a trailing element to place content at the end of the field, before the
built-in clear and toggle buttons. Unlike the leading element, this position is
suited to controls — an inline filter button, for example:
...
A trailing control keeps its own behavior: pressing it runs its handler without
opening the options popover or clearing the selection. Mark decorative content
`aria-hidden="true"`; interactive content needs its own `aria-label`.

Keep the field readable — a leading icon, a trailing control, the clear button
and the toggle button can all appear at once, so add only what earns its place.
