...
A menu is a list of options or actions that appears when a user interacts with a
trigger element, providing contextual actions in a compact interface.

## Overview

Menus provide users with a list of actions or options, typically accessed
through a button or icon button trigger. They help organize secondary actions
and reduce visual clutter by keeping options hidden until needed.
...
The basic menu consists of a trigger (usually a button) and content that appears
when activated.
...
Enhance menu items with descriptions and keyboard shortcuts for better user
experience.
...
Menus support single selection mode where only one item can be selected at a
time.
...
Combine different selection modes within a single menu by applying selection
settings to individual sections.
...
Menus provide a space-efficient way to display a list of actions or options,
appearing on demand when users interact with a trigger element.
...
- **Predictable Placement:** Position menus consistently to avoid covering
  important content
- **Appropriate Triggers:** Choose triggers that clearly indicate a menu will
  appear
...
- **Keyboard Navigation:** Full keyboard support with arrow keys, Enter, and
  Escape
- **Selection Modes:** Support for single and multiple selection with visual
  indicators
- **Flexible Triggers:** Can be triggered by any interactive element using
  `asChild`
- **Sections & Separators:** Organize items into logical groups
- **Submenus:** Support for nested menu structures
- **Rich Content:** Items can include icons, descriptions, and keyboard
  shortcuts
...
<GroupedPropsTable componentName="Menu" />

## Accessibility

The Menu component is built with accessibility in mind, providing full keyboard
