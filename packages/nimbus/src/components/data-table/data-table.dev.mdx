...
Each row must have a unique `id` property. It is the key for selection,
`disabledKeys`, expansion and pinning. To identify rows by another property,
set `id` in the row data, for example `rows.map((v) => ({ ...v, id: v.sku }))`.
Additional properties are accessed via column `accessor` functions:
...
The `size` prop controls cell padding, header padding and text size. `sm`,
`md` and `lg` use the same values as the `Table` component. Choose a smaller
size to show more data on one screen:

| Size | Cell padding (horizontal / vertical) | Text |
| ---- | ------------------------------------ | ---- |
| `sm` | 8px / 8px                            | 14px |
| `md` | 12px / 12px                          | 14px |
| `lg` | 16px / 12px                          | 16px |
...
**Default size.** Without a `size` prop, the table uses `xl`: 24px / 16px cell
padding and a 40px header, the appearance from before `size` existed. `xl` is
deprecated. Do not pass it explicitly (it logs a warning in development); use
`lg` instead. A future major release will change the default.

**Cell text.** `sm`, `md` and `lg` set the text size of the cells. If your cell
renderers set their own size, for example `<Text textStyle="sm">`, remove it so
the content follows the table's size.

**Internal columns.** The drag, selection, expand and pin controls stay 24×24px
at every size, the minimum target size of WCAG 2.2 SC 2.5.8. Only the padding
around them follows the size.

**Deprecated `density`.** `density="condensed"` still works when no `size` is
passed, but is deprecated: use `size="lg"` or `size="md"` instead. When both
are passed, `size` wins and `density` is ignored. `density` will be removed in
...
pinning behavior. Do not set `id` on `DataTable.Row`; the row's identity comes
from the row data, and a different `id` logs a development warning:
...
`DataTable.Manager` loads on demand. Its dependencies (Drawer, Tabs,
DraggableList) are fetched the first time you render it, instead of being part
of the main DataTable download. The component brings its own loading boundary,
so you do not need to add one. Two things follow from this: the settings button
appears one frame after the table, and tests must query it with an asynchronous
query such as `await screen.findByRole("button", { name: /settings/i })` rather
than `getByRole`.
...
> [!NOTE]\
> The row density setting still controls the deprecated `density` prop and
> cannot choose a `size` yet. A size control for the Layout Settings tab is
