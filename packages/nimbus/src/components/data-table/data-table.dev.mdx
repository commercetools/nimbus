...
Use `renderDetails` to show a full-width detail panel below a row when clicked.
The panel toggles open and closed on each click. This works alongside selection
and nested rows.
...
const App = () => {
  ...
  return (
    <Stack gap="300">
      <Text>Click a row to expand its detail panel.</Text>
      <DataTable
        columns={columns}
        rows={rows}
        renderDetails={(row) => (
...
const App = () => {
  ...
  return (
    <Stack gap="300">
      <Text>Click a row to expand its nested content.</Text>
      <DataTable
        columns={columns}
        rows={rows}
        nestedKey="children"
