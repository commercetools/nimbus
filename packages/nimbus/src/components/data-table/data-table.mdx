...
To show more data in smaller spaces, choose a smaller size. The sizes `sm`,
`md` and `lg` match the sizes of the Table component: they change the padding
of cells and headers and the text size. The drag, selection, expand and pin
controls keep their 24×24px size, so they stay easy to hit.

Without a size, a data table keeps its previous, most spacious appearance.
...
const App = () => {
  ...
  return <Stack gap={16}>
    {['sm', 'md', 'lg'].map((size) => (
      <DataTable key={size} rows={data} columns={columns} size={size} />
    ))}
  </Stack>
}
```
