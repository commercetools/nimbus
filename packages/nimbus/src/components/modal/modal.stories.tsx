  ...
  play: async ({ canvasElement, step }) => {
    ...
    for (const size of sizes) {
      await step(`Opens ${size} modal and verifies accessibility`, async () => {
        ...
        const dialog = await canvas.findByRole("dialog", { name: `Size: ${size}` });
        expect(dialog).toBeInTheDocument();
  ...
  play: async ({ canvasElement, step }) => {
    ...
    for (const placement of placements) {
      await step(`Tests ${placement} placement modal`, async () => {
        ...
        const dialog = await canvas.findByRole("dialog", { name: `Placement: ${placement}` });
        expect(dialog).toBeInTheDocument();
...
export const ScrollBehavior: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Tests inside scroll behavior", async () => {
      ...
      const dialog = await canvas.findByRole("dialog", { name: "Scroll: inside" });
      expect(dialog).toBeInTheDocument();
...
export const MotionPresets: Story = {
  ...
  render: () => (
    <Stack direction="row" wrap>
      {(["scale", "slide-in-bottom", "slide-in-top", "slide-in-left", "slide-in-right", "none"] as const).map((preset) => (
...
export const WithoutBackdrop: Story = {
  ...
  render: () => (
    <Modal.Root>
      <Modal.Trigger>
        <Button>Open Modal (No Backdrop)</Button>
      </Modal.Trigger>
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Opens modal without backdrop", async () => {
      const trigger = canvas.getByRole("button", { name: "Open Modal (No Backdrop)" });
      await userEvent.click(trigger);
      
      const dialog = await canvas.findByRole("dialog", { name: "No Backdrop Modal" });
      expect(dialog).toBeInTheDocument();
...
export const ControlledState: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Controls modal state externally", async () => {
      const trigger = canvas.getByRole("button", { name: "Open Controlled Modal" });
      await userEvent.click(trigger);
      
      const dialog = await canvas.findByRole("dialog", { name: "Controlled Modal" });
      expect(dialog).toBeInTheDocument();
...
export const KeyboardNavigation: Story = {
    ...
    <Modal.Root>
      ...
      <Modal.Content>
        ...
        <Modal.Header>
          ...
          <Modal.CloseTrigger>
            <Button variant="ghost" size="xs" aria-label="Close modal">
              ×
            </Button>
          </Modal.CloseTrigger>
        </Modal.Header>
        <Modal.Body>
          <Stack>
            <Modal.Description>
              ...
              Escape to close, Enter/Space on buttons.
            </Modal.Description>
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Tests keyboard interactions", async () => {
      const trigger = canvas.getByRole("button", { name: "Test Keyboard Navigation" });
      await userEvent.click(trigger);
      
      const dialog = await canvas.findByRole("dialog", { name: "Keyboard Navigation Test" });
      expect(dialog).toBeInTheDocument();
