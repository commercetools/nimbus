...
Integrate LocalizedField with form libraries like Formik:
...
const MyForm = () => {
  const initialValues: FormValues = {
    productName: LocalizedField.createLocalizedString(['en', 'de', 'es'], {}),
    description: LocalizedField.createLocalizedString(['en', 'de', 'es'], {})
  };
      ...
      {({ values, setFieldValue, setFieldTouched, touched, errors }) => (
        <Form>
          <LocalizedField
            ...
            name="productName"
            defaultLocaleOrCurrency="en"
            valuesByLocaleOrCurrency={values.productName}
            onChange={(e) => {
              setFieldValue(
                LocalizedField.getName('productName', e.target.locale!),
                e.target.value
              );
            }}
            onBlur={(_, locale) => {
              setFieldTouched(
                LocalizedField.getName('productName', locale),
                true
              );
            }}
            touched={LocalizedField.isTouched(touched.productName)}
          ...
          <LocalizedField
            ...
            onChange={(e) => {
              setFieldValue(
                LocalizedField.getName('description', e.target.locale!),
                e.target.value
              );
            }}
            onBlur={(_, locale) => {
              setFieldTouched(
                LocalizedField.getName('description', locale),
                true
              );
            }}
            touched={LocalizedField.isTouched(touched.description)}
...
```tsx
import { LocalizedField, type LocalizedCurrency } from '@commercetools/nimbus';

const PricingForm = () => {
  const [prices, setPrices] = React.useState<LocalizedCurrency>({
    USD: { amount: '', currencyCode: 'USD' },
    EUR: { amount: '', currencyCode: 'EUR' },
    GBP: { amount: '', currencyCode: 'GBP' },
    JPY: { amount: '', currencyCode: 'JPY' }
  });
...
const ProductNameField = () => {
  const [values, setValues] = React.useState<LocalizedString>({
    en: '',
    de: '',
    es: '',
    'fr-FR': ''
  ...
  const validateLocale = (locale: string, value: string) => {
    // English is required
    if (locale === 'en') {
  ...
  return (
    <LocalizedField
      ...
      onChange={(e) => {
        setValues(prev => ({
          ...prev,
          [e.target.locale!]: e.target.value as string
        }));
      }}
      onBlur={(_, locale) => {
