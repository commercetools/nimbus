...
- **Focus management**: Focus moves into the modal on open and returns to the trigger on close.
- **Focus containment**: Tab navigation stays within the modal while it is open.
- **Accessible names**: `ModalPage.Title` renders an `h2` with `slot="title"`, automatically linked as `aria-labelledby`.
