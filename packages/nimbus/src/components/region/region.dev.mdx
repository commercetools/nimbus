...
- **`Region.Root`** — establishes a named-region scope. Wrap it around the
...
`null` (node/value) before an outlet mounts or without a `Region.Root` ancestor,
...
`Region.Root` may wrap an entire application, so publishing into a region must
...
1. **External store (primitive).** The registry is read via
   ...
   consumers; `Region.Root` itself never re-renders.
