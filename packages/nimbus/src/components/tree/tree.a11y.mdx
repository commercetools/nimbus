...
- **Drag and drop:** when drag-and-drop is enabled, include a
  `Button slot="drag"` per row so reordering is operable by keyboard and screen
  reader (pick up with `Enter`, move with arrow keys, drop with `Enter`), not
  pointer-only.
