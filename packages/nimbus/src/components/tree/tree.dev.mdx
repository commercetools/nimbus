...
Add an `IconButton` (or `Button`) with `slot="drag"` inside `Tree.ItemContent`
so keyboard and screen reader users can pick items up (`Enter` to grab, arrow
keys to move, `Enter` to drop). Drop indicators for valid positions are provided
automatically.

```tsx
import { Tree, useTree, IconButton } from "@commercetools/nimbus";
    ...
    <Tree.Item id={node.key} textValue={node.value.title}>
      <Tree.ItemContent>
        <IconButton
          ...
          variant="ghost"
          size="2xs"
          aria-label={`Reorder ${node.value.title}`}
        >
          <DragIndicator />
        </IconButton>
        <Tree.Indicator />
