...
<Tree.Root>
  <Tree.Item>
    ...
    {/* nested <Tree.Item>s or a <Collection> */}
  </Tree.Item>
</Tree.Root>
```
...
For data-driven trees, pass an `items` array and a recursive render function,
using React Aria's `Collection` to render each node's children.
...
const items = [
  ...
  },
  {
    id: "photos",
    title: "Photos",
    children: [{ id: "image-1", title: "Image 1" }],
  },
];
...
- React Aria implements the tree as an ARIA `treegrid` — a screen-reader-tested
  pattern for interactive trees that support selection and per-row actions. Rows
  expose `aria-level`, `aria-expanded` and `aria-selected`.
...
- Keyboard: Up/Down move between rows, Right/Left expand/collapse (or move to
  child/parent), Home/End jump to first/last, and typing performs type-ahead.
- When drag-and-drop is enabled, include a `<Button slot="drag">` per item so
  the reorder is operable by keyboard and screen reader, not just pointer.
