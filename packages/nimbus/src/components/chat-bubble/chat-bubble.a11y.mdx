...
`ChatBubble` renders a **single message**. It intentionally does not own the
...
`ChatBubble.Root` renders a semantic `<article>` by default (a feed item, per
...
`ChatBubble.Avatar` sets `aria-hidden` unless you pass `firstName`/`lastName` or
...
`ChatBubble.Root`'s default element already satisfies that — and don't mix in
non-article children.

### Streaming state

`isStreaming` on `ChatBubble.Root` sets `aria-busy="true"`, flagging the
...
and use `ChatBubble.Typing`'s `children` for a visible "Assistant is typing…"
...
- **Name, role, value:** `ChatBubble.Root` is an `<article>` by default; always
