...
- \`side="bottom"\` → placement="bottom", motionPreset="slide-in-bottom"
  ...
  argTypes: {
    ...
    size: {
      control: 'select',
      options: ['xs', 'sm', 'md', 'lg', 'xl', 'narrow', 'wide', 'cover', 'full'],
  ...
  <>
    ...
    <Drawer.Body>
      <nav style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <a href="#" style={{ padding: '0.5rem', textDecoration: 'none', borderRadius: '4px' }}>
          🏠 Dashboard
        </a>
        <a href="#" style={{ padding: '0.5rem', textDecoration: 'none', borderRadius: '4px' }}>
          👤 Profile
...
const DetailContent = ({ side = 'right' }) => (
  <>
    ...
    <Drawer.Body>
      <Drawer.Description>
        Detailed information about the selected product including specifications, reviews, and availability.
      </Drawer.Description>
    ...
    <Drawer.Footer>
      <button style={{ padding: '0.5rem 1rem', border: '1px solid #ccc', borderRadius: '4px', backgroundColor: 'white' }}>
        Add to Wishlist
      </button>
      <button style={{ padding: '0.5rem 1rem', border: 'none', borderRadius: '4px', backgroundColor: '#007bff', color: 'white' }}>
        Add to Cart
      </button>
    </Drawer.Footer>
  </>
...
const NotificationContent = ({ side = 'top' }) => (
  <>
    ...
    <Drawer.Header>
      ...
      <Drawer.CloseTrigger aria-label="Close notifications">×</Drawer.CloseTrigger>
    </Drawer.Header>
    <Drawer.Body>
...
export const RightDetails: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText(/Detailed information about the selected product/)).toBeInTheDocument();
...
export const TopNotifications: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText('System update available')).toBeInTheDocument();
      ...
      <Drawer.Content side={args.side} size={args.size}>
        ...
        <Drawer.Body>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <button style={{ padding: '0.5rem', textAlign: 'center' }}>🔧</button>
            <button style={{ padding: '0.5rem', textAlign: 'center' }}>📊</button>
            <button style={{ padding: '0.5rem', textAlign: 'center' }}>⚙️</button>
            <button style={{ padding: '0.5rem', textAlign: 'center' }}>📝</button>
          </div>
        </Drawer.Body>
      </Drawer.Content>
      ...
      <Drawer.Content side={args.side} size={args.size}>
        ...
        <Drawer.Body>
          <div style={{ padding: '2rem' }}>
            ...
            <p>
              This drawer takes up the full screen, perfect for immersive experiences
      ...
      <div>
        ...
        <Drawer.Root isOpen={isOpen} onOpenChange={setIsOpen}>
          <Drawer.Content side={args.side} size={args.size}>
            ...
            <Drawer.Body>
              <p>
                This drawer's open state is controlled by external state.
                You can open/close it from buttons outside the drawer.
              </p>
...
export const ScrollableContent: Story = {
  ...
  render: (args) => (
    <Drawer.Root {...args}>
      ...
      <Drawer.Content side={args.side} size={args.size} scrollBehavior={args.scrollBehavior}>
        <Drawer.Backdrop />
        <Drawer.Header>
          ...
          <div>
            {Array.from({ length: 50 }, (_, i) => (
              <p key={i} style={{ marginBottom: '1rem' }}>
                This is paragraph {i + 1} of long scrollable content. 
