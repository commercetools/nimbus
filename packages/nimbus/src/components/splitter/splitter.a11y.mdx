...
- The handle is keyboard-reachable in DOM order and operable with the arrow
  ...
  splitter is `isDisabled`. While a pane is collapsed, the resize keys (arrows,
  Home/End) are inactive — Enter still toggles collapse — because a collapsed
  pane sits below its `minSize` and has no valid in-range position to resize to.
