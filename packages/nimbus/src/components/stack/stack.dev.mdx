...
Stack re-exports Chakra UI's Stack component.

### `direction`

Controls the layout direction of stacked children.
...
A React element to render between each child item.

```tsx
<Stack separator={<Box height="25" bg="neutral.6" />}>
  {/* Separator appears between each child */}
</Stack>
```

Stack also supports all [Chakra UI styling props](https://www.chakra-ui.com/docs/styled-system/style-props) including `gap`, `align`, `justify`, `wrap`, spacing, colors, borders, and responsive values.
  ...
  <Stack direction="column" gap="600" align="stretch">
    ...
    <Box>
      ...
      <Stack direction="row" gap="100">
        <IconButton icon={Edit} aria-label="Edit" size="xs" variant="ghost" />
        <IconButton icon={Copy} aria-label="Copy" size="xs" variant="ghost" />
        <IconButton icon={Share} aria-label="Share" size="xs" variant="ghost" />
        <IconButton icon={Delete} aria-label="Delete" size="xs" variant="ghost" colorPalette="critical" />
      </Stack>
    </Box>
  </Stack>
)
      ...
      <Card.Content>
        <Stack direction="column" gap="300">
          <Stack direction="row" justify="space-between" align="center">
            ...
            <IconButton icon={MoreVertical} aria-label="More options" size="xs" variant="ghost" />
          </Stack>
