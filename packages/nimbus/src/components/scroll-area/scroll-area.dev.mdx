...
Wrap content in a `ScrollArea` with a size constraint. The scrollbar appears
when the pointer enters the area or when the content scrolls, then hides again
...
Use the `scrollbarVisibility` prop to choose when the scrollbar is shown.
`auto-hide` (the default) reveals the bar on pointer entry or scroll and fades
it out after a short idle delay; while the pointer rests inside, scrolling or
moving toward the bar reveals it again, and resting the pointer on the bar keeps
it shown. `always` keeps the bar visible and reserves a gutter so it never
overlays content:
...
Use the `variant` prop for the scrollbar's visual style — `solid` (default),
`inset`, `overlay`, or `glass`. It is independent of `scrollbarVisibility`, so
any look can also be permanently visible:
...
```jsx live-dev
const App = () => (
  <ScrollArea maxH="200px" w="400px" bg="neutral.2" borderRadius="300" scrollbarVisibility="always" p="200">
    {Array.from({ length: 20 }, (_, i) => (
      <Text key={i} fontSize="sm">
        Line {i + 1}: Lorem ipsum dolor sit amet, consectetur adipiscing elit.
      </Text>
