...
- **Auto-hide is visual only:** With the default `hover` variant the bar hides
  after a short idle delay, but this never affects scrollability — wheel,
  touch, and keyboard scrolling keep working while it is hidden, and scrolling
  reveals it again. Use `scrollbarVisibility="always"` where a permanently
  visible bar is preferred.
