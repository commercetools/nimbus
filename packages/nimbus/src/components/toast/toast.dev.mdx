...
```tsx
import { toast, Toast, type ToastOptions } from '@commercetools/nimbus';
```

### Basic usage

The Toast component provides two ways to create notifications: an imperative API for programmatic toasts and compound components for custom layouts. The imperative API is the recommended approach for most use cases.
...
const App = () => (
  <Stack direction="row" gap="200" flexWrap="wrap">
    ...
    <Button onPress={() => toast({ title: "Top", placement: "top" })}>
      Top
    </Button>
    ...
    <Button onPress={() => toast({ title: "Bottom", placement: "bottom" })}>
      Bottom
    </Button>
