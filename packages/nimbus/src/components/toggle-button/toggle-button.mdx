---
id: Components-ToggleButton
title: Toggle button
exportName: ToggleButton
description: An interactive button that toggles between two binary states such as active and inactive.
lifecycleState: Stable
order: 999
menu:
  - Components
  - Buttons
  - Toggle button
tags:
  - component
  - buttons
  - toggle
  - controls
figmaLink: >-
  https://www.figma.com/design/AvtPX6g7OGGCRvNlatGOIY/NIMBUS-design-system?node-id=5707-16294&m=dev
---

## Overview

A toggle button is used similarly to a checkbox or a radio button, but supports
a smaller interactive component that can be used to give a binary active state
choice to users that can be used alone or in combination with other choices. A
toggle button or toggle icon button gives a user a new way of visualizing the
information in the interface. It does not typically perform an action. Please
use a button or an icon button for these uses.

### Resources

Deep dive into implementation details and access the Nimbus design library.

[Figma library](https://www.figma.com/design/AvtPX6g7OGGCRvNlatGOIY/NIMBUS-design-system?node-id=5707-16294&m=dev)

## Variables

Get familiar with the features.

### Visuals

#### Outlined

Most common styling of the toggle button.

```jsx live
const App = () => (
  <ToggleButton variant="outline">
    <Icons.PushPin />
    Product categories
  </ToggleButton>
)
```

#### Ghost

Ghost styled buttons are helpful for UI simplification.

```jsx live
const App = () => (
  <ToggleButton variant="ghost">
    <Icons.PushPin />
    Product categories
  </ToggleButton>
)
```

#### Subtle

A filled neutral chip at rest — useful when the control should read as an
affordance even before it is selected.

```jsx live
const App = () => (
  <ToggleButton variant="subtle">
    <Icons.PushPin />
    Product categories
  </ToggleButton>
)
```

### Active fill

The `activeFillStyle` prop sets the weight of the active (selected) fill,
independent of the resting `variant`. Use `tint` (default) for multi-select sets
and `solid` when a single choice should stand out.

```jsx live
const App = () => (
  <Stack gap="400" align="center" direction="row">
    <ToggleButton colorPalette="primary" activeFillStyle="tint" isSelected>
      Tint (default)
    </ToggleButton>
    <ToggleButton colorPalette="primary" activeFillStyle="solid" isSelected>
      Solid
    </ToggleButton>
  </Stack>
);
```

### Color palette

The resting toggle button is always neutral; `colorPalette` tints the active
(selected) state, so each palette below is shown unselected and selected.

#### Primary (default)

The default colorPalette for most toggle buttons.

```jsx live
const App = () => (
  <Stack gap="400" align="center" direction="row">
    <ToggleButton>
      Primary
    </ToggleButton>
    <ToggleButton isSelected>
      Selected
    </ToggleButton>
  </Stack>
);
```

#### Neutral

Use for secondary or less prominent toggles.

```jsx live
const App = () => (
  <Stack gap="400" align="center" direction="row">
    <ToggleButton colorPalette="neutral">
      Neutral
    </ToggleButton>
    <ToggleButton colorPalette="neutral" isSelected>
      Selected
    </ToggleButton>
  </Stack>
);
```

#### Critical

Use for toggles that represent destructive or warning states.

```jsx live
const App = () => (
  <Stack gap="400" align="center" direction="row">
    <ToggleButton colorPalette="critical">
      Critical
    </ToggleButton>
    <ToggleButton colorPalette="critical" isSelected>
      Selected
    </ToggleButton>
  </Stack>
);
```

#### Info

Use for informational or help-related toggles.

```jsx live
const App = () => (
  <Stack gap="400" align="center" direction="row">
    <ToggleButton colorPalette="info">
      Info
    </ToggleButton>
    <ToggleButton colorPalette="info" isSelected>
      Selected
    </ToggleButton>
  </Stack>
);
```
