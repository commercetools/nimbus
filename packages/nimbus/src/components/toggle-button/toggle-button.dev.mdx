...
match your interface density — the same scale as `Button`:
...
The `variant` prop controls the resting chrome. In every variant the resting
button is neutral — `colorPalette` is reserved for the active (selected) state
(see "Color palettes" below). Choose the variant that matches your surface:

- `outline` (default): a neutral border over a transparent fill.
- `ghost`: no border and no fill until hover — good for dense toolbars.
- `subtle`: a filled neutral chip at rest.
...
const App = () => (
  <Stack direction="column" gap="400">
    ...
    <Stack direction="row" gap="400" wrap="wrap">
      <ToggleButton variant="subtle">
        Subtle
      </ToggleButton>

      <ToggleButton variant="subtle" isSelected>
        Subtle Selected
      </ToggleButton>
    </Stack>
  </Stack>
...
`activeFillStyle` controls the weight of the active (selected) fill. It is
independent of `variant`, which only styles the resting state.

- `tint` (default): a light accent wash — `colorPalette.5` fill with
  `colorPalette.12` text.
- `solid`: a full accent fill — `colorPalette.9` fill with contrast text.

Use `tint` for multi-select sets where several buttons can be on at once, and
`solid` when a single choice should stand out.
...
const App = () => (
  <Stack direction="column" gap="400">
    ...
    <Stack direction="row" gap="400" wrap="wrap">
      <ToggleButton colorPalette="primary" activeFillStyle="tint" isSelected>
        Primary tint
      </ToggleButton>

      <ToggleButton colorPalette="primary" activeFillStyle="solid" isSelected>
        Primary solid
      </ToggleButton>
    </Stack>
  </Stack>
...
`colorPalette` tints the **active (selected)** state — the resting button stays
neutral, so the palette only shows once a button is selected. Use different color
palettes to indicate different types of actions:
