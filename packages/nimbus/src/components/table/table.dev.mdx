...
Enable hover highlighting with the `interactive` prop. This provides visual feedback when users hover over rows:
...
const App = () => {
  ...
  return (
    <Stack gap="400">
      {selectedRow && (
        <Text>Selected: {selectedRow.name} ({selectedRow.role})</Text>
      )}
...
Enable alternating row backgrounds with the `striped` prop for improved readability:

```jsx-live-dev
const App = () => {
  const inventory = [
    { sku: 'SKU-001', product: 'Laptop', quantity: 45, location: 'Warehouse A' },
    { sku: 'SKU-002', product: 'Monitor', quantity: 120, location: 'Warehouse B' },
    { sku: 'SKU-003', product: 'Keyboard', quantity: 200, location: 'Warehouse A' },
    { sku: 'SKU-004', product: 'Mouse', quantity: 350, location: 'Warehouse C' },
    { sku: 'SKU-005', product: 'Headset', quantity: 80, location: 'Warehouse B' },
  ];
...
The Table component is re-exported from Chakra UI v3. Each part accepts standard HTML table element props plus Chakra style props.

### Table.Root

Main container that provides styling and configuration for the entire table.

**Props:**
- `variant`: `"line"` | `"outline"` - Visual style (default: `"line"`)
- `size`: `"sm"` | `"md"` | `"lg"` - Size variant (default: `"md"`)
- `interactive`: boolean - Enables hover highlighting on rows
- `striped`: boolean - Alternating row backgrounds
- `showColumnBorder`: boolean - Vertical borders between columns
- `stickyHeader`: boolean - Keeps header fixed during scroll
- All HTMLChakraProps for `<table>` element

### Table.Header

Container for the table header row(s). Renders `<thead>`.

### Table.Body

Container for the table data rows. Renders `<tbody>`.

### Table.Footer

Container for footer rows (summary, totals). Renders `<tfoot>`.

### Table.Row

A table row. Renders `<tr>`.

### Table.ColumnHeader

A header cell. Renders `<th>`. Use for column headers to provide semantic structure.

### Table.Cell

A data cell. Renders `<td>`.

### Table.Caption

Table caption for providing a title or description. Renders `<caption>`.

### Table.ScrollArea

Wrapper for scrollable tables. Use with `stickyHeader` to keep headers visible.
...
- [Storybook - Table](https://nimbus-storybook.vercel.app/?path=/docs/components-table--docs)
- [Chakra UI v3 - Table Documentation](https://chakra-ui.com/docs/components/table)
- [DataTable Component](../data-table) - Advanced table with sorting, selection, and more features
