...
The leading search icon is the default, and can be replaced with any element or
removed entirely by passing `null`. Use `trailingElement` to place content at the
end of the field, before the clear button — an inline filter button, for example:
...
const App = () => (
  <Stack direction="column" gap="400">
    <SearchInput
      ...
      aria-label="Default leading icon"
    />
    <SearchInput
      placeholder="Replaced leading icon"
      aria-label="Replaced leading icon"
      leadingElement={<Icons.Visibility aria-hidden="true" />}
    />
    <SearchInput
      placeholder="No leading element"
      aria-label="No leading element"
      leadingElement={null}
    />
    <SearchInput
...
Decorative content should be marked `aria-hidden="true"`. Interactive content
needs its own `aria-label`, and keeps its own click behavior — pressing a
trailing button does not clear the field or move focus to the input.
