...
`variant` changes the track/progress/thumb treatment. `plain` (the default) is a
standard filled track; `enclosed` is a thick, contained bar with the thumbs inset
inside it. This applies to both `RangeSlider` and `Slider` since they share the
same recipe.

```jsx live-dev
const App = () => (
  <Stack direction="column" gap="600">
    <RangeSlider aria-label="Plain" variant="plain" defaultValue={[20, 60]} />
    <RangeSlider
      aria-label="Enclosed"
      variant="enclosed"
      defaultValue={[25, 75]}
