...
- **Rely on the value tooltip, not extra UI.** The current value already
  appears above the handle on hover, focus, or drag — there's no need to add a
  separate number readout next to the slider.
- **Use `RangeSlider` for bounded ranges.** Its two handles can't cross, so it
  guarantees the lower value never exceeds the upper one — useful for filters
  like price or distance ranges.
...
A slider lets users choose a value — or, with `RangeSlider`, a range — by
dragging a handle along a track, best for values that are set by feel rather
than typed precisely.
...
> - Filtering by a numeric range, such as price, distance, or rating.
...
> - Use `RangeSlider` when both a lower and upper bound need to be set
>   together.
...
> - Add a separate "current value" label next to the track — the value
>   tooltip already covers that on hover, focus, and drag.
