...
`ChatMessage.Body` in `ChatMessage.Root`. `sender` defaults to `"assistant"`.
...
`sender="user"` trails the avatar and uses the `primary.3` surface;
`sender="assistant"` (the default) leads with the avatar. Name a message with
...
Tool / function-call output is content _inside_ an `assistant` message (there is no
...
generation, so an `assistant` message can be both `sender="assistant"` and
`tone="error"`.
