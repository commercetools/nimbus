...
- You need a custom `aria-label` on a non-string title
...
When the `title` is not a plain string, consider providing an explicit `aria-label` via the escape hatch below if the composed markup does not form a meaningful accessible name on its own.
...
If the `title` is a composed ReactNode, consumers remain responsible for any extra affordances their composition needs (for example, an explicit `aria-label` on the trigger or via the escape hatch below).

## Escape hatch

If you need a non-default size, a custom dismiss behaviour, or a custom accessible label, drop down to composing `Dialog` directly. The equivalent manual composition is:
