...
When the confirm handler is async, hold `isConfirmLoading` true for the duration. The pattern automatically:

- Shows a spinner inside the confirm button
- Disables both buttons
- Suppresses Escape, overlay click, and the X button
...
- **Confirm button** → `onConfirm()` then `onOpenChange(false)` (does NOT invoke `onCancel`)
