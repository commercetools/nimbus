...
- Building standard forms with monetary fields
...
```tsx
import { MoneyInputField, type MoneyInputFieldProps } from '@commercetools/nimbus';
...
MoneyInput automatically detects and displays a badge for high precision amounts:
...
The high precision badge appears when the amount has more decimal places than the currency standard (e.g., USD typically uses 2 decimals, so "99.999" triggers the badge).
...
type CurrencyCode = "USD" | "EUR" | "GBP" | "JPY" | ... // 100+ currencies
```
...
The MoneyInputField handles most accessibility requirements internally through FormField and MoneyInput. The `label` prop is required and ensures proper labeling for screen readers.
