...
The DateRangePickerField is a convenience pattern that combines a DateRangePicker with FormField, providing built-in labels, descriptions, error handling, and validation feedback. It reduces boilerplate and ensures consistency across your forms.
...
**With DateRangePickerField** (3 lines):
```tsx
<DateRangePickerField
...
```jsx-live-dev
const App = () => {
  const [valueSm, setValueSm] = useState(null);
...
Control field interactivity with state props:
...
DateRangePickerField uses the `FieldErrors` component to display validation errors with built-in localized messages.
...
DateRangePickerField provides comprehensive accessibility through FormField:
...
**No additional ARIA attributes needed** - FormField handles all accessibility relationships automatically.

- Info button appears next to the label when `info` prop is provided
- Supports ReactNode, allowing rich content (text, lists, links)
- Accessible via keyboard (`Tab` to focus, `Enter` / `Space` to open)
