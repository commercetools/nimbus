 ...
 *   Pass an explicit path in monorepo CI setups where `process.cwd()` differs
 *   from the application root being built.
 *
 * Uses the provided `cwd` as the `createRequire` base instead of
 * `import.meta.url` because Rolldown's CJS output transpiles
 * `import.meta.url` to `{}.url` (undefined), which breaks `createRequire`.
 */
export function isNimbusResolvable(cwd: string = process.cwd()): boolean {
  try {
