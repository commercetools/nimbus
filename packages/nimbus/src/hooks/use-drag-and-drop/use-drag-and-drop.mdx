...
> [!IMPORTANT]
> `getKey` must return the same key the collection itself uses, because that is
> what React Aria hands back to the handlers.
>
> The default is `(item) => item.key ?? item.id`. **`DataTable` keys rows by
> `row.id`**, so pass `getKey` explicitly there. Domain rows commonly carry a
> business `key` field as well — customer groups, categories, product types,
> channels and stores all do — and the default would prefer it, so reordering
> would match nothing and silently do nothing:
>
> ```tsx
> // ✅ DataTable rows are keyed by id
> createArrayHandlers(setRows, (row) => row.id)
>
> // ❌ picks up a business `key` field if the rows have one
> createArrayHandlers(setRows)
> ```
>
> In development, a mismatch logs a warning naming the cause and the fix.
