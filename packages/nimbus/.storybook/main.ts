    ...
    return mergeConfig(config, {
      resolve: {
        alias: isDevelopment
          ...
          : // Production build & production test (test:storybook): no alias —
            // the package resolves to the built dist via node_modules.
            // (Dev test pins source itself in vitest.storybookdev.config.ts.)
            {},
      },
    });
  },
