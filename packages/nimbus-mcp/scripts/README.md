...
| `copy-docs-data.ts`            | Mirrors generated docs data from `apps/docs/src/data` into `data/docs` (clears each destination first, so upstream deletions propagate) |
...
| `build-style-props-summary.ts` | Extracts style prop names from docs sub-pages into `data/style-props-summary.json`                                                      |
