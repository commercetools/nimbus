...
- ❌ Any imports from `@storybook/blocks`
- ❌ Using `<Canvas>`, `<Controls>`, `<ArgTypes>` components
- ❌ Missing required frontmatter fields: `id`, `title`, `description`, `menu`,
  `tags`
- ✅ MUST use `jsx-live` code blocks for examples
- ✅ MUST use `<PropsTable id="ComponentName" />` for props

### Component File Organization

- ❌ Utility functions in component root (must be in `utils/`)
- ❌ Hooks outside of `hooks/` directory
- ❌ Constants not in `constants/` directory
- ❌ Missing barrel exports in `index.ts`

### Stories Files

- ❌ Missing `play` functions for interactive components
- ❌ No interaction tests for user-interactive features
- ❌ Missing comprehensive prop combinations

### Common Violations

- ❌ Missing JSDoc comments
- ❌ Incorrect import conventions (React Aria must use `Ra` prefix)
- ❌ Side effects in utility functions
- ❌ Mutable constants (missing `as const`)
