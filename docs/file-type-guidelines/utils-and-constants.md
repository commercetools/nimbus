...
[← Back to Index](../component-guidelines.md) | [Previous: Hooks](./hooks.md) | [Next: Context Files →](./context-files.md)

## Purpose

The `utils/` and `constants/` directories contain helper functions, utilities, configuration constants, and enums specific to a component. They help organize non-React logic and static values.
...
### 1. React Logic in Utils

```typescript
// ❌ Bad - uses React hooks in util
// utils/helpers.ts
export function useHelper() {  // This is a hook!
...
// ✅ Good - React hook in hooks folder
// hooks/use-helper.ts
...
CONFIG.value = 20; // Can be modified!

// ✅ Good - immutable with 'as const'
export const CONFIG = {
  value: 10,
} as const;
// CONFIG.value = 20; // Error: Cannot assign to 'value'
```
...
// ✅ Good - documented
/**
 * Process data items by adding processed flag
 * @param data - Array of data items
 * @returns Processed data with added flag
 */
export function processData<T>(data: T[]): (T & { processed: boolean })[] {
  return data.map(d => ({ ...d, processed: true }));
}
```

### 4. Side Effects in Utils
...
// Let component handle the side effect
```
