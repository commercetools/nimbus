...
await userEvent.click(trigger);
const menu = canvas.getByRole("menu"); // May fail
...
### 1. Missing Play Functions

```typescript
// ❌ Bad - interactive component without play function
export const Interactive: Story = {
  args: {
...
### 2. No Progressive Complexity

```typescript
// ❌ Bad - jumping to complex examples
export const ComplexExample: Story = {
  // Complex setup without simpler examples
};
...
### 3. Inadequate Async Handling

```typescript
// ❌ Bad - no waiting for async operations
await userEvent.click(trigger);
expect(canvas.getByRole("menu")).toBeInTheDocument(); // May fail
