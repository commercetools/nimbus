...
### 1. Hook in Wrong Location

```typescript
// ❌ Bad - hook in component root
component-name/
├── use-component.ts  # Wrong!
└── component-name.tsx

// ✅ Good - hook in hooks folder
component-name/
├── hooks/
│   └── use-component.ts  # Correct!
└── component-name.tsx
```

### 2. Missing use Prefix
...
```typescript
// ❌ Bad - any types
export function useComponent(options: any): any {
  /* ... */
}

// ✅ Good - proper types
export function useComponent(options: UseComponentOptions): UseComponentReturn {
  /* ... */
}
```

### 4. Side Effects Without Cleanup
