...
// ❌ Avoid - recipe variants (use RecipeVariantProps instead)
...
// ❌ Avoid - redundant explicit variant declarations
...
### 1. Missing Property Documentation

```typescript
// ❌ Bad - no property documentation
export interface ButtonProps {
  variant?: string;
  isLoading?: boolean; // Missing JSDoc!
}

// ✅ Good - all properties documented
export interface ButtonProps
  extends ComponentProps<"button">,
    RecipeVariantProps<typeof buttonRecipe> {
  // variant inferred from recipe
...
// ✅ Also Good - with optional interface JSDoc for complex cases
/**
 * Configuration for advanced button behaviors
 */
export interface ButtonAdvancedProps {
  ...
  /**
   * Delay before action executes (in ms)
   * @default 0
   */
  actionDelay?: number;
}
...
### 2. Wrong Base Interface

```typescript
// ❌ Bad - using HTMLAttributes
...
### 4. Internal Types in Public API File

```typescript
// ❌ Bad - internal types in public API file
// component-name.types.ts
export interface ComponentNameProps {
  // Public API - correct
}

export interface InternalSanitizationOptions {
  // Only used by internal sanitization function - wrong location!
  allowUnsafeContent?: boolean;
  internalCache?: Map<string, any>;
}

// ✅ Good - internal types colocated with usage
// component-name.types.ts (PUBLIC API ONLY)
export interface ComponentNameProps {
  // Public API - correct location
}

// utils/sanitize.ts (INTERNAL TYPES COLOCATED)
interface SanitizationOptions {
  // Used only within this utility - correct location
  allowUnsafeContent?: boolean;
  internalCache?: Map<string, any>;
}
```

### 5. Redundant Recipe Variant Declarations

```typescript
// ❌ Bad - redundant explicit variant declarations
export interface ButtonProps
  extends ComponentProps<"button">,
    RecipeVariantProps<typeof buttonRecipe> {
  variant?: "solid" | "outline" | "ghost"; // Already inferred!
  size?: "sm" | "md" | "lg"; // Already inferred!
  isLoading?: boolean;
}

// ✅ Good - rely on automatic inference
export interface ButtonProps
  extends ComponentProps<"button">,
    RecipeVariantProps<typeof buttonRecipe> {
  // variant and size automatically available from recipe
  isLoading?: boolean;
}

// ✅ Exception - functional overlap requires explicit declaration
export interface TabsProps
  extends ComponentProps<"div">,
    RecipeVariantProps<typeof tabsRecipe> {
  /**
   * Affects both styling AND keyboard navigation behavior
   */
  orientation?: "horizontal" | "vertical";
}
```
