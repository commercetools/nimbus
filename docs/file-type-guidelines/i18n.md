...
For messages with variables, pass the arguments as the third parameter:

```typescript
const { locale } = useLocale();

// Use getVariableLocale for variable messages
// Always returns a string (empty string if message not found)
const label = componentMessages.getVariableLocale("avatarLabel", locale, {
  fullName: user.name,
});
```
...
- [ ] Component calls `componentMessages.getVariableLocale("key", locale)` for
      simple string messages (no args needed)
- [ ] Component calls
