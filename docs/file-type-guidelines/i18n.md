...
For messages with variables, use optional chaining to safely call the function:

```typescript
const { locale } = useLocale();

// Use getVariableLocale for variable messages
// Note: getVariableLocale is only available on components that have messages with variables
// Returns undefined if message not found, so optional chaining (?.) is required
const label = componentMessages.getVariableLocale(
  "avatarLabel",
  locale
)?.({
  fullName: user.name,
...
- [ ] Component calls `componentMessages.getVariableLocale("key", locale)` for
      variable messages (only available when component has variable messages)
