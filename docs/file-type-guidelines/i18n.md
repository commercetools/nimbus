...
2. Translation workflow → Transifex translates and opens a PR with updated
   `data/*.json` files → compile and merge the PR manually
...
> [NimbusProvider documentation](../../packages/nimbus/src/components/nimbus-provider/nimbus-provider.mdx#locale)**
...
> **Note:** Replace `{componentName}` with the actual component name in
> camelCase. For example: `alertMessagesStrings`, `drawerMessagesStrings`,
> `splitButtonMessagesStrings`.
...
```mermaid
flowchart TD
    Start["🎯 Component calls<br/>useLocalizedStringFormatter hook"]

    Start --> Hook["⚙️ Hook internally calls<br/>useLocale() from react-aria-components"]

    Hook --> Context["📦 Reads from React Aria<br/>I18nProvider context"]

    Context --> CheckNested{"Nearest<br/>NimbusI18nProvider?"}

    CheckNested -->|Yes - Nested| NestedProvider["🔹 Nested NimbusI18nProvider<br/>(wraps React Aria's I18nProvider)"]

    CheckNested -->|No - Check Parent| CheckParent{"Parent<br/>NimbusProvider?"}

    CheckParent -->|Yes| ParentProvider["🔹 Parent NimbusProvider<br/>→ NimbusI18nProvider (receives locale prop)<br/>→ Wraps React Aria's I18nProvider"]

    CheckParent -->|No Provider| Browser["🌐 Browser default<br/>(navigator.language)"]

    NestedProvider --> Resolved["✅ Locale Resolved"]
    ParentProvider --> Resolved
    Browser --> Resolved

    style Start fill:#e3f2fd,color:#000000
    style Hook fill:#fff3e0,color:#000000
    style Context fill:#f3e5f5,color:#000000
    style NestedProvider fill:#c8e6c9,color:#000000
    style ParentProvider fill:#c8e6c9,color:#000000
    style Browser fill:#ffccbc,color:#000000
    style Resolved fill:#b2dfdb,color:#000000
```

**Precedence Order (Highest to Lowest Priority):**

1. **Nested NimbusI18nProvider** locale prop (most specific)
2. **Nested NimbusProvider** locale prop
3. **Parent NimbusProvider** locale prop
4. **Browser locale** (navigator.language) - fallback
...
- [ ] Component imports message strings correctly:
      `import { {componentName}MessagesStrings } from "./{component-name}.messages"`
      (e.g., `alertMessagesStrings`, `drawerMessagesStrings`)
...
- [Accessibility Guidelines](./architecture-decisions.md) - Accessibility
  requirements
- [React Aria Integration](./architecture-decisions.md#when-to-use-react-aria) -
